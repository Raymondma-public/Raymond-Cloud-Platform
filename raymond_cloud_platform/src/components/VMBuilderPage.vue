<template>
  <div>
    <a-row type="flex">
      <a-col :span="6">
        vmCode
        <a-input id="vmCode" type="text" name="vmCode" v-model="vmCode"></a-input>
      </a-col>
    </a-row>
    ...
    <a-row type="flex">
      <a-col :span="12">
        dockerBuildString
        <a-textarea
          id="dockerBuildString"
          type="text"
          name="dockerBuildString"
          rows="10"
          cols="60"
          v-model="dockerBuildString"
        ></a-textarea>
      </a-col>
    </a-row>
...
export default {
  ...
  methods: {
    getVMConfig(event) {
      ...
      const key = "getVMConfig";
      this.$message.loading({ content: "Loading...", duration: 0, key });
      axios
        .get("http://192.168.1.240:30084/getVMConfig", { params: data })
        .then(res => {
        ...
        .catch(error => {
          this.$message.error({ content: "Error!", key, duration: 2 });
          console.error(error);
        })
        .finally(() => {
          /* 不論失敗成功皆會執行 */
    ...
    buildVM(event) {
      let data = {
        token: this.token,
        vmCode: this.vmCode,
        dockerBuildString: this.dockerBuildString,
      ...
      this.$message.loading({ content: "Loading...", duration: 0, key });
      axios
        .post("http://192.168.1.240:30084/buildVM", formData)
        .then(res => {
          this.$message.success({
            content: res.data.message,
            key,
